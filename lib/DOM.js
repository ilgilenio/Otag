...
let proto = {
  Element: {
    ...
    do(method, on = 'click', args) {
      if(arguments[3]) {
        args = O.toArray(arguments).splice(2)
      }
      this['on' + on] = () => {
  ...
  SVGElement: {
    set(src, height, width) {
      ...
      O.req(src).then(i => {
        ...
        O.toArray(tmp.attributes).forEach(attr =>
  ...
  Element: {
    ...
    str: {
      get() {
        ...
        str += (this.id != '' ? '#' + this.id : '')
        str += O.toArray(this.classList).map(i => '.' + i)
          .join('')
        O.toArray(this.attributes).forEach((i) => {
